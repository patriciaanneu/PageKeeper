    ...
    <header className='sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-gray-200'>
      <div className='max-w-6xl mx-auto px-4 py-4 sm:py-5 flex items-center relative'>
        ...
        <div className='flex items-center gap-4'>
          <div className='hidden sm:flex items-center gap-4'>
            {user ? (
              <>
                <NavLink to='/books' className={({isActive}) => isActive ? 'text-gray-900 underline text-base' : 'text-gray-600 hover:text-gray-900 text-base'}>My Books</NavLink>
                <NavLink to='/books/new' className={({isActive}) => isActive ? 'text-gray-900 underline text-base' : 'text-gray-600 hover:text-gray-900 text-base'}>Add</NavLink>
                <button onClick={handleLogout} className='text-gray-600 hover:text-gray-900 text-base'>Logout</button>
                <NavLink to='/profile' className='text-gray-600 hover:text-gray-900 text-base'>{user.username}</NavLink>
              </>
            ) : (
              <>
      ...
      <div className={`sm:hidden border-t border-gray-100 bg-white transition-all duration-200 ease-out transform origin-top ${open ? 'max-h-96 opacity-100 scale-100' : 'max-h-0 opacity-0 scale-95 pointer-events-none'}`}>
        <div className={`px-6 ${open ? 'py-4' : 'py-0'} space-y-3 overflow-hidden`}>
          ...
          {user ? (
            <>
              <NavLink onClick={()=>setOpen(false)} to='/books' className={({isActive}) => isActive ? 'block text-gray-900 underline text-base' : 'block text-gray-700 hover:text-gray-900 text-base'}>My Books</NavLink>
              <NavLink onClick={()=>setOpen(false)} to='/books/new' className={({isActive}) => isActive ? 'block text-gray-900 underline text-base' : 'block text-gray-700 hover:text-gray-900 text-base'}>Add</NavLink>
              <button onClick={handleLogout} className='block text-left w-full text-gray-700 hover:text-gray-900 text-base'>Logout</button>
              <NavLink onClick={()=>setOpen(false)} to='/profile' className={({isActive}) => isActive ? 'block text-gray-900 underline text-base' : 'block text-gray-700 hover:text-gray-900 text-base'}>{user.username}</NavLink>
            </>
          ) : (
            <>
              ...
              <NavLink onClick={()=>setOpen(false)} to='/register' className={({isActive}) => isActive ? 'block bg-[#594A3E] text-white px-4 py-2 rounded text-base' : 'block bg-[#594A3E] text-white px-4 py-2 rounded hover:opacity-95 text-base'}>Sign Up</NavLink>
            </>
          )}
        </div>
      </div>
